...
public class QRDecomposition {
     ...
     * @param b The right-hand side target org.jjophoven.math.Vector.
     * @return The solution org.jjophoven.math.Vector x.
     */
    public Vector solve(Vector b) {
        int m = Q.getRows();
        int n = Q.getColumns();
        if (b.size() != m) {
            throw new IllegalArgumentException("org.jjophoven.math.Vector size must match matrix row count.");
        }
        ...
        for (int i = n - 1; i >= 0; i--) {
            ...
            if (Math.abs(R.get(i, i)) < 1e-9) {
                throw new ArithmeticException("org.jjophoven.math.Matrix is singular or near-singular.");
            }
            x[i] = sum / R.get(i, i);
        }
